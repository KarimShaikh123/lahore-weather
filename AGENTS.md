...
- `js/app.js` — fetches `READINGS_URL` (currently `./sample-reading.json` — the mock; **task 9 changes this to `/api/readings`**), handles loading/error/staleness; `?sample=<name>` loads `./sample-<name>.json` (dev preview only — `sample-reading-night.json` shows the night theme); `startTicker` updates the Lahore clock (`#clock`, `Intl` + `Asia/Karachi`) every 30s and the staleness line every 60s.
