...
- [x] Task 12 — Test everything (2026-08-13): full `node:test` suite 25/25 + live wrong-input pokes against the real handlers (`/tmp/opencode/task12-pokes.js`): ingest GET→405, no/bad auth→401, wrong-station→502, garbage weather body→502, waqi status error→502; readings POST→405, GET→200 with real latest + history array
- [x] Task 13 — Deploy + final review (2026-08-13): live at **https://lahore-weather-one.vercel.app** (project `lahore-weather`, deployment `lahore-weather-npbm3f5dn-personal-e375`). `AQI_API_KEY` + `CRON_SECRET` added to Vercel Production & Preview env (Development only allows non-sensitive — `.env.local` covers local dev); GH secrets `INGEST_URL` (= live `/api/ingest`) + `CRON_SECRET` set. Manual `workflow_dispatch` run **succeeded** (HTTP 200) and stored a fresh reading — the hourly cron is live. Content-verified the live page (title, Lahore, real readings with gases). Note: Open-Meteo `current.time` ticks every 15 min, so a cron run stores the current 15-min reading; dedup key is the hour, so repeated runs within an hour overwrite rather than duplicate
- [x] Sitemap (2026-08-20): `sitemap.xml` at the root — single URL (`https://lahore-weather-one.vercel.app/`, the only public page); served at `/sitemap.xml` (cleanUrls does not touch `.xml`). Submit in Search Console → Sitemaps
- [x] Google Search Console verification (2026-08-20): root file `google5bfb335e102f0ca7.html` — content is the single line `google-site-verification: google5bfb335e102f0ca7.html`, byte-for-byte as Google issued. Note: `cleanUrls: true` 308s `/google5bfb335e102f0ca7.html` → `/google5bfb335e102f0ca7`; same-domain redirects are supported by Search Console (verified against Google's docs 2026-08-20). Do not rename, reformat, or add a trailing newline — Google matches content exactly
- [x] Data guards (2026-08-20, audit): freshness now asserted for WAQI (`data.time.iso`) and CAMS (`current.time`) too, not just Open-Meteo; dedup score floored to the hour (history truly hourly, `-168` = 7 days, docs true); `sensors_disagree` cross-check (EPA-inverse implied pm25 vs CAMS, flag in UI via `#aqi-disagree`, never reject — Karim's call); throwaway cleanup removed pre-fix pm1 members from Redis. 34/34 tests
- [x] Forecast timezone fix (2026-08-20, audit): Open-Meteo hourly `time` is naive-local; `new Date` parsed it as browser-local, so viewers outside UTC+5 saw shifted hour labels. `buildForecastHours` now stamps `utc_offset_seconds` (fallback 18000) onto each time; tests pin the instant and pass under TZ=UTC/Karachi/New_York (29/29)
- [x] Particle-source fix (2026-08-17): live probe proved WAQI `iaqi` pm1/pm10 stuck provider-side (154/77 unchanged for days, impossible vs the live pm25) and `iaqi.pm25` == `aqi` (US-AQI sub-index, not µg/m³); Open-Meteo rejects `pm1` as a variable entirely. Particles now from CAMS (`pm2_5`→pm25, `pm10`→pm10, µg/m³), pm1 dropped from the stored reading, WAQI keeps aqi + dominantpol. Tests: mapping asserts CAMS values + pm1 absent; new test pins iaqi-particle ignorance; readings fixture de-impossibilized. Shipped as PR #2, merged + verified live (pm25 44.1 / pm10 64.3 µg/m³ sane, pm1 gone)
...
- **Particle trap (fixed 2026-08-17): the station's `iaqi` pm1/pm10 were stuck at 154/77 for days on the provider side (physically impossible vs the live pm25), and `iaqi.pm25` is a US-AQI sub-index — equal to `aqi`, not µg/m³. Particles now come from CAMS: `pm2_5`→`pm25`, `pm10`→`pm10` (µg/m³; live-verified 2026-08-17: 45.5/65.4). `pm1` has no trustworthy source (Open-Meteo rejects `pm1` as a variable; WAQI's is stale) — the key is no longer stored; the UI never rendered it. WAQI now contributes only `aqi` + `dominentpol` (gases still station-wins over CAMS).**
...
```json
{
  ...
  "sensors_disagree": false
}
```

Pollutants are optional keys (`pm25`, `pm10`, `no2`, `o3`, `so2`, `co`) — a provider may not supply all of them; render missing ones as `—`. `pm1` is no longer stored (no trustworthy source since the 2026-08-17 particle-source fix).

### Frontend (task 4)

- `js/data.js` — pure logic: `aqiCategory`, `weatherCodeLabel`, `formatStaleness`, `weatherIconKey` (WMO → icon key), `aqiPosition` (AQI → 0–100% for the scale bar), `formatOffset` (seconds → ISO offset like `+05:00`, mirrors `api/ingest.js`), `buildForecastHours(hourly, utcOffsetSeconds)` (hourly arrays → forecast slots, each `time` stamped with the offset — missing offset falls back to 18000 — so `new Date` parses a real instant in any viewer timezone, not browser-local), `sparklinePath` (values → SVG polyline path, null when <2 finite points); browser global `window.LWData`, Node export for tests. WMO thunder is only 95–99 (999 must land on `unknown`).
...
- `js/render.js` — `LW.render(reading, $)` fills the DOM ids from a reading; unhides `#aqi-disagree` when `reading.sensors_disagree` (station AQI and CAMS pm25 diverge — both still shown as reported); AQI colour via `data-cat`; inline-SVG weather icon from `LW_ICONS` into `#w-icon` (`data.weather` = icon key); sets `#aqi-marker` `left` to `aqiPosition%`; sets `document.body.dataset.theme` to `day`/`night` from `reading.is_day` (1 = day) and `document.body.dataset.sky` to a sky class (`clear`/`partly`/`overcast`/`storm`, mapped from the icon key) — the two attributes drive the atmospheric background gradient. `LW.renderHistory(history, $)` draws the AQI trend sparkline (sets `#aqi-spark-path` `d` from `sparklinePath`, writes the min–max range + current AQI, unhides `#trend`); no-ops when history has < 2 readings.
- `js/app.js` — fetches `READINGS_URL = "/api/readings"` (task 9, was `./sample-reading.json`), uses `payload.latest` (the API returns `{ latest, history }`, not a flat reading); when `latest` is null it shows "No readings yet — the hourly collector will fill this in." instead of a crash; calls `LW.renderHistory(payload.history, $)` for the AQI trend; handles loading/error/staleness; `startTicker` updates the Lahore clock (`#clock`, `Intl` + `Asia/Karachi`) every 30s and the staleness line every 60s. `sample-reading.json` still exists but is now only the contract fixture — nothing reads it at runtime.
- `js/forecast.js` — client-side, keyless (added 2026-08-13): fetches Open-Meteo `hourly=temperature_2m,precipitation_probability,weather_code,is_day&forecast_hours=24&timezone=auto` directly from the browser (CORS verified — Open-Meteo only sends `access-control-allow-origin` when the request carries an Origin header, which browsers always do) and renders the `#forecast` strip; rain % is shown only when > 0. Passes `payload.utc_offset_seconds` into `LWData.buildForecastHours(hourly, utcOffsetSeconds)` (tested) so hour labels are Lahore wall clock for every viewer — the 2026-08-20 fix for naive-ISO parsed as browser-local; the module only wires DOM.
- `google5bfb335e102f0ca7.html` — Google Search Console verification file; content-exact, do not touch (see status).
- `sitemap.xml` — single-URL sitemap for Search Console; add a `<url>` entry here if the site ever gains pages.
...
- Script load order in `index.html` matters: `data.js` → `icons.js` → `render.js` → `app.js` → `forecast.js`.
- Run tests: `npm test` (the `test` script is `node --test`).

### api/ serverless functions (tasks 6–8)

- `api/ingest.js` — POST only. Bearer `CRON_SECRET` auth (`isAuthorized`); 405 on non-POST, 401 on bad auth, 502 on provider/validation failure, 500 on internal, 200 `{ ok, recorded_at, score, count }` on success. Fetches Open-Meteo weather (explicit `temperature_unit=celsius&wind_speed_unit=kmh&timezone=auto`), WAQI station feed, and Open-Meteo air-quality (CAMS: `pm10,pm2_5,carbon_monoxide,nitrogen_dioxide,sulphur_dioxide,ozone`) in parallel; validates all three (unit assertion °C/%/km/h, ranges, `is_day` 0/1, station-offline via `data:null`, wrong-city reject, freshness within ±2h/1h for ALL three sources — weather naive+offset, WAQI `data.time.iso`, air naive+offset; a stalled provider 502s, never stores silently); builds the stored-reading contract (particles pm25/pm10 from CAMS µg/m³; gases station-wins with CAMS fallback; WAQI contributes aqi + dominantpol only; `sensors_disagree` = EPA-inverse cross-check, see below); score is `hourScore` (epoch floored to the hour — repeat runs within an hour overwrite, history is truly hourly); writes `zremrangebyscore(readings, score, score)` → `zadd` → `zremrangebyrank(readings, 0, -721)` prune.
- **Keep the Redis client construction inside the handler** — tests `require("../api/ingest.js")` and must not construct a client (env vars may be unset in CI). Pure helpers are exported on `module.exports`: `isAuthorized`, `toEpochSeconds`, `formatOffset` (seconds → ISO offset like `+05:00`), `buildReading` (stamps `recorded_at` as `current.time + formatOffset(utc_offset_seconds)` so timestamps carry their offset), `validateWeather`, `validateWaqi`, `validateAir` (CAMS — only rejects a field when present and bad), `validateFreshness` (naive-local + offset, label param), `validateFreshnessIso` (offset-stamped ISO like WAQI's), `hourScore`, `impliedPm25` (US EPA breakpoint inverse), `sensorsDisagree` (flags when implied-vs-CAMS pm25 diverge by >20 µg/m³ AND ratio >2.5 — Karim's call 2026-08-20: flag in UI, never reject).
